...
test("styles use the approved resident dashboard palette", async () => {
  ...
  assert.match(css, /@media\s*\(max-width:\s*520px\)\s*\{[\s\S]*\.hero-actions\s*\{[\s\S]*grid-template-columns:\s*1fr;/s);
});
...
test("footer visitor counters use shared countapi keys and right-aligned layout", async () => {
  ...
  assert.match(js, /fetch\("\.\/api\/visitor-stats"\)/);
  assert.doesNotMatch(js, /https:\/\/api\.countapi\.xyz/);
  assert.doesNotMatch(js, /hotdeal-visitor-total-counted/);
  assert.doesNotMatch(js, /hotdeal-visitor-day/);
  assert.match(api, /https:\/\/visitor\.6developer\.com\/visit\?domain=/);
  assert.match(api, /module\.exports = async \(req, res\) => \{/);
  assert.match(api, /setCommonHeaders\(res\)/);
  assert.match(api, /todayCount/);
  assert.match(api, /totalCount/);
